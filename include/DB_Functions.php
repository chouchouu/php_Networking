...
class DB_Functions{
	...
	public function checkUser($email)
	{
		...
		if($rows>0) 
			...
			public function fashion($tenfs,$giafs,$motafs)
			{
				$sql="insert into fashion
				(tenfs,giafs,motafs) values
				('$tenfs','$giafs','$motafs')";
				$con=$this->db->connect();
				$result=mysqli_query($con,$sql);
				return $result;
		...
		// 	public function updateProduct($id,$tenfs,$giafs,$motafs)
		// 	{
		// 		$sql="update fashio set tenfs='$tenfs',
		// 			giafs='$giafs',motafs='$motafs' where id='$id'";
		// 		$con=$this->db->connect();
		// 		$result=mysqli_query($con,$sql);
		// 		return $result;
		// 	}
